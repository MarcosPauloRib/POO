public class Aluno {
    private String nome;
    private int matricula;
    private double notaAV1;
    private double notaAV2;
    private double notaAE;
    private String curso;
    private int periodo;

    public Aluno(String nome, int matricula, double notaAV1, double notaAV2, double notaAE, String curso, int periodo) {
        this.nome = nome;
        this.matricula = matricula;
        this.notaAV1 = notaAV1;
        this.notaAV2 = notaAV2;
        this.notaAE = notaAE;
        this.curso = curso;
        this.periodo = periodo;
    }

    public void alteraNotaAV1 (int nota){ notaAV1 = notaAV1 + nota;}

    public void alteraNotaAV2 (int nota){ notaAV2 = notaAV2 + nota;}

    public void alteraNotaAE (int nota){ notaAE = nota;}

    public String avaliarAluno(){
        if (notaAV1 + notaAV2 >= 60){
            return "Aprovado!";
        }
        else{
            return "Recuperação!";
        }
    }

    public String avaliarRecuperacao(){
        if(notaAV1 + notaAV2 + notaAE / 2 >= 60){
            return "Aprovado!";
        }
        else{
            return "Reprovado!";
        }
    }
}
